...
public class DaYunCanvas extends PanCanvas{
	public DaYunCanvas(Composite parent, Pan pan) {
		super(parent, pan);
	...
	private void paintBlock(GC gc){
		 for(int i=0; i<_blockArea.size(); i++){
			 ...
			 for(int j=0; j<zhengYaoList.size(); j++){
				 ...
				 if(yao.daYunSiHua >= 0)
				 {
					 String sihua = Pan.siHua[yao.daYunSiHua];
			 ...
			 List<String> yunYaoList = block.yunYaoList;
			 for(int j=0; j<yunYaoList.size(); j++){
				 String yunYao = yunYaoList.get(j);
				 gc.drawString(yunYao, x, y);
				 y += 20;
			 }
			 gc.setForeground(_display.getSystemColor(SWT.COLOR_BLACK));
			 //画宫干支
			 ...
			 y = blockArea.y + blockArea.height - 20;
			 gc.drawString(block.daYunShiErGong, x, y);
			 //画博士十二宫
			 x = blockArea.x + 5;
